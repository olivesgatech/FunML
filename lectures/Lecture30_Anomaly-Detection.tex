...
thresholds, we use the \emph{Receiver Operating Characteristic (ROC) curve} \cite{4}.
...
distribution as a weighted combination of Gaussian components \cite{2}.
...
anomalies are both rare and different, making them easier to isolate \cite{3}.
...
in-distribution samples and lower confidence for unfamiliar or anomalous inputs \cite{6}.
...
\section{Related Problem Formulations}
...
\hline
\textbf{Problem} & \textbf{Training Data Assumption} & \textbf{Goal} & \textbf{Key Characteristic} \\
\hline

\textbf{Anomaly Detection} 
& Normal data with possible contamination or unlabeled data 
& Detect rare deviations from normal patterns 
& Broad umbrella setting; anomalies may be diverse and partially observed  \\
\hline

\textbf{Novelty Detection} 
& Only normal data available during training 
& Detect new but consistent deviations from normal 
& Assumes anomalies are not seen during training \\
\hline

\textbf{Out-of-Distribution (OOD) Detection} 
& Labeled training data from known classes 
& Detect inputs outside training distribution 
& Focus on distribution shift relative to training data \\
\hline

\textbf{Misprediction Detection} 
& Fully supervised model 
& Detect when model predictions are likely incorrect 
& Relies on model confidence or uncertainty estimates \\
\hline

\textbf{Adversarial Detection} 
& Standard training data (clean inputs) 
& Detect maliciously perturbed inputs 
& Inputs are intentionally crafted to fool the model \\
\hline

\textbf{Open Set Recognition} 
& Labeled known classes 
& Classify known classes while rejecting unknown ones 
& Combines classification with rejection of unseen classes \\
\hline

\end{tabular}
\caption{Comparison of anomaly detection and related problem formulations.}
\label{tab:anomaly_related_tasks}
\end{table}
...
\item \textbf{Question:} 
Can a sample be \textbf{highly uncertain} for a classifier but \textbf{not} be an anomaly? Explain the difference.

\textbf{Solution:}  
Yes. A sample can lie near the decision boundary between two classes, making the classifier uncertain (e.g., similar probabilities for multiple classes). However, such a sample may still belong to the normal data distribution and therefore is not an anomaly.

Uncertainty reflects ambiguity in classification, while anomaly detection focuses on deviations from the underlying data distribution. Thus, uncertain samples are often informative and valid, whereas anomalies are rare and structurally different from normal data.

\item \textbf{Question:} 
What is the key difference between \textbf{anomaly detection}, \textbf{OOD detection}, and \textbf{open set recognition}?

\textbf{Solution:}  
Anomaly detection focuses on identifying rare or unusual samples relative to a learned notion of normality, often without relying on labeled classes.  

OOD detection assumes a supervised setting and aims to detect inputs that fall outside the training distribution of known classes.  

Open set recognition extends classification by requiring the model to both classify known classes and reject samples from unseen classes.  

While all three involve detecting unfamiliar inputs, they differ in their assumptions about training data and the role of classification.

\item \textbf{Question:} 
Why might out-of-distribution (OOD) samples be problematic in active learning?

\textbf{Solution:}  
OOD samples may exhibit high uncertainty, which could cause them to be selected by uncertainty-based acquisition functions. However, labeling such samples may not improve the model because they do not belong to the target data distribution. As a result, they can waste annotation budget and potentially degrade performance.
