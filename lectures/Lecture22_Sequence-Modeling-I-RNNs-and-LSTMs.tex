...
\lecture{19}{Sequence Modeling I: RNNs and LSTMs}{Ghassan AlRegib and Mohit Prabhushankar}{}

\noindent\textbf{Keywords:} Sequence modeling, recurrent neural networks, long short-term memory, gated recurrent units, language modeling
...
To minimize this sequence-level objective, RNNs are trained using Backpropagation Through Time (BPTT), a temporal extension of backpropagation developed for recurrent systems \cite{2}. The network is first \emph{unrolled} across the entire sequence, forming a deep computational graph with one layer per time step. Gradients are then computed by applying the chain rule backward through this unfolded network.
