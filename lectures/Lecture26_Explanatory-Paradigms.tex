...
\lecture{23}{Trustworthy ML II: Explanatory Paradigms}{Ghassan AlRegib and Mohit Prabhushankar}{}

%%%%%CHANGE HERE%%%%%%%
%%%%%\section{title of the section} similarly with the rest \section{} or \subsection{} or \subsubsection{} etc

\noindent\textbf{Keywords:} Explainability, feature attribution, contrastive explanation, counterfactual explanation, saliency maps, evaluation benchmarks, performance metrics
...
Dimensionality reduction techniques like t-SNE can be applied to the final embeddings of a model, projecting high-dimensional features into 2D space to reveal how well the model clusters data points from different classes \cite{3,4}.
...
\end{center}
\caption{Examples of indirect explanation techniques, including maximally activated patches, activation visualizations, dimensionality reduction, and nearest-neighbor retrieval.}
...
\end{center}
\caption{Examples of direct explanation techniques: saliency via occlusion and saliency via feature importance.}
...
Targeted explanations \cite{11} focus on answering specific questions about a model’s decision-making, often tailored to a particular user, task, or scenario. Unlike general explanations that provide an overall view of model behavior, targeted explanations are precise and question-driven \cite{8}. In machine learning, especially in image classification, these explanations commonly address three key questions: 
...
Contrastive explanations \cite{12} answer questions such as ``Why Spoonbill rather than Flamingo or Fox?'' by emphasizing differences between the predicted class and plausible alternatives.
...
attends to when asked about an \emph{unrelated} class  \cite{7}. For example, given an image
...
\begin{figure}[H]
  ...
  \caption{Grad-CAM workflow. Gradients are aggregated to weight convolutional feature maps, producing a heatmap that highlights image regions supporting the queried class.}
...
This provides insight into how strongly the model depends on evidence for class \(P\) and what alternative features might support other predictions \cite{7,8}.
...
\caption{Counterfactual-CAM workflow. Gradients are negated to suppress features contributing to class $P$, revealing alternative regions that could support other classes if $P$ were absent.}
...
\caption{Comparison of explanation types. Contrast-CAM highlights regions that differentiate a predicted class from alternative classes, revealing discriminative and confusing features.}
...
Contrast-CAM can also explain model uncertainty by contrasting the predicted class with competing classes collectively (e.g., the next highest-scoring alternatives).
...
Explanatory evaluation can be categorized into three complementary types based on the nature of the tasks and the role of human involvement \cite{8}.
...
\textbf{Application Evaluation} focuses on tasks that intersect with explainability but do not require humans to explicitly judge explanations. Instead, explanation quality is inferred through its impact on application-level metrics (e.g., testing whether a noisy image is still classified correctly) \cite{8}.
...
models align with human visual perception at scale \cite{10}. Such comparisons
...
\begin{figure}[H]
    ...
    \caption{Saliency maps compared across multiple explanation and saliency detection methods applied to the same images.}
...
\caption{Gaze-tracking example illustrating human visual attention on salient image regions.}
\label{fig:gaze_tracking_example}
\end{figure}

To quantitatively compare explanation maps with human attention, several 
metrics are commonly used \cite{10}:
...
\caption{Pointing game illustrating human-indicated regions of importance in images.}
...
identify image regions that truly influence the model’s predictions \cite{8}.
...
Backpropagation} or \emph{Grad-CAM} \cite{9}.

\begin{figure}[H]
\centering
\includegraphics[width=4in]{img/lecture23/net2.png}
\caption{Masking using explanation heatmaps to evaluate feature importance.}
...
or dataset mean), and the modified image is re-evaluated by the network \cite{9}.

\begin{figure}[H]
\centering
\includegraphics[width=4in]{img/lecture23/net3.png}
\caption{Pixel-wise deletion: progressively masking pixels from highest to lowest importance.}
...
Figures~\ref{fig:deletion_auc_examples} and \ref{fig:deletion_more_examples}) \cite{9}.
...
\caption{Deletion curves: stronger explanations cause faster confidence drop, yielding lower AUC.}
...
\caption{Additional deletion examples illustrating the effect of importance ranking on confidence decay.}
...
informative pixels early, causing confidence to rise quickly \cite{9}.
...
\centering
\includegraphics[width=4in]{img/lecture23/net6.png}
\caption{Insertion curves: stronger explanations cause faster confidence rise, yielding higher AUC.}
...
\textbf{Solution:}  
\begin{itemize}
    \item \textbf{Indirect explanation:} dimensionality reduction (e.g., t-SNE/PCA) on embeddings to visualize clustering structure. It provides auxiliary insight into learned representations without directly attributing importance to inputs \cite{8}.
...
In \emph{European Conference on Computer Vision (ECCV)}, pages 818--833, 2014.

\bibentry{2}
K. Simonyan, A. Vedaldi, and A. Zisserman.
Deep Inside Convolutional Networks: Visualising Image Classification Models and Saliency Maps.
In \emph{International Conference on Learning Representations Workshop (ICLR Workshop)}, 2014.
...
In \emph{International Conference on Learning Representations (ICLR Workshop)}, 2015.

\bibentry{6}
B. Zhou, A. Khosla, A. Lapedriza, A. Oliva, and A. Torralba.
Learning Deep Features for Discriminative Localization.
In \emph{IEEE Conference on Computer Vision and Pattern Recognition (CVPR)}, pages 2921--2929, 2016.

\bibentry{7}
R. R. Selvaraju, M. Cogswell, A. Das, R. Vedantam, D. Parikh, and D. Batra.
Grad-CAM: Visual Explanations from Deep Networks via Gradient-Based Localization.
In \emph{IEEE International Conference on Computer Vision (ICCV)}, pages 618--626, 2017.
...
In \emph{British Machine Vision Conference (BMVC)}, 2018.

\bibentry{10}
M. K\"ummerer, T. S. A. Wallis, and M. Bethge.
DeepGaze II: Reading Fixations from Deep Features Trained on Object Recognition.
arXiv:1610.01563, 2016.

\bibentry{11}
AlRegib, Ghassan, and Mohit Prabhushankar. "Explanatory paradigms in neural networks: Towards relevant and contextual explanations." \emph{IEEE Signal Processing Magazine}, 39.4 (2022): 59-72.

\bibentry{12}
Prabhushankar, Mohit, Gukyeong Kwon, Dogancan Temel, and Ghassan AlRegib. "Contrastive explanations in neural networks." In \emph{2020 IEEE International Conference on Image Processing (ICIP)}, pp. 3289-3293. IEEE, 2020.
