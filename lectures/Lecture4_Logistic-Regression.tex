...
\lecture{3}{Classification II: Logistic Regression}{Ghassan AlRegib and Mohit Prabhushankar}{}

%%%%%CHANGE HERE%%%%%%%
%%%%%\section{title of the section} similarly with the rest \section{} or \subsection{} or \subsubsection{} etc

\noindent\textbf{Keywords:} Binary classification, Logistic regression, classification, sigmoid, softmax, gradient descent, decision boundaries
...
\subsection{Linear Classification as a Decision Boundary}

A central idea in classification is that many models make decisions by separating the input space into regions corresponding to different classes. One of the simplest and most important ways to do this is through a \textbf{linear classifier} \cite{1}.

A linear classifier is based on a linear function of the form:
\[
f_{\mathbf{w}}(\mathbf{x}) = \mathbf{w}^T \mathbf{x} + b,
\]
where $\mathbf{w}$ is a weight vector and $b$ is a bias term.

For binary classification, predictions are made using the sign of this function \cite{1}:
\[
\hat{y} = \text{sign}(\mathbf{w}^T \mathbf{x} + b).
\]

This means that the classifier assigns inputs to one of two classes depending on which side of a \textbf{decision boundary} they fall on. The decision boundary is defined by:
\[
\mathbf{w}^T \mathbf{x} + b = 0,
\]
which represents a line in 2D, a plane in 3D, and a hyperplane in higher dimensions.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/lecture3/linear-boundary.png}
    \caption{A two-dimensional example of linear classification. Multiple separating hyperplanes may exist (left), and a learned classifier selects a decision boundary that separates the two classes (right), adapted from \cite{1}.}
    \label{fig:linear_boundary}
\end{figure}

\noindent
\textbf{Figure:} A two-dimensional example of linear classification. Multiple separating hyperplanes may exist (left), and a learned classifier selects a decision boundary that separates the two classes (right).
...
\begin{itemize}
    \item $\mathbf{w}^T \mathbf{x} + b > 0$ $\rightarrow$ Class 1
    \item $\mathbf{w}^T \mathbf{x} + b < 0$ $\rightarrow$ Class 0 (or $-1$)
\end{itemize}
...
Although linear regression also uses a linear function, it is not well-suited for classification tasks \cite{1}. The squared loss used in regression treats prediction errors as numeric differences, which can lead to undesirable behavior.
...
This highlights a key issue: classification requires modeling \textbf{discrete decisions}, not continuous errors.

\subsubsection{Towards Better Classification Models}
...
\bibentry{1}
N. Osa, R. Johnson, T. Engelder, and N. Taheri, ``Lecture 4: Linear Classifiers II,'' COMP 642 Machine Learning, Rice University, Jan. 19, 2023. Available: \url{https://www.cs.rice.edu/~as143/COMP642_Spring23/Scribes/jan_19.pdf}
