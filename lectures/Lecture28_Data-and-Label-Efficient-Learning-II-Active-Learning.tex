...
\lecture{25}{Data and Label Efficient Learning II – Active Learning}{Ghassan AlRegib and Mohit Prabhushankar}{}
...
\noindent\textbf{Keywords:} Active learning, uncertainty quantification, calibration, performance metrics, representation learning

\section{Lecture Objectives}

The objective of this lecture is to introduce the principles of data and label efficient learning through active learning and to understand how machine learning models can strategically select informative samples under limited annotation budgets \cite{1}. Students will learn the differences between supervised learning and active learning workflows, understand the role of acquisition functions, and study uncertainty-based methods such as entropy, least confidence, and margin sampling, as well as diversity-based approaches such as core-set selection and discriminative active learning \cite{3,4}. Additionally, this lecture aims to develop an understanding of how fusion strategies balance diversity and difficulty, how active learning performance is evaluated using both model metrics and annotation cost metrics, and how deployment considerations such as performance regression, negative flips, and Negative Flip Rate (NFR) affect the reliability of iterative model updates.
...
Active learning changes this assumption. Instead of requiring all labels in advance, active learning assumes access to a large pool of unlabeled data together with a limited annotation budget. The learner is allowed to interactively choose which samples should be labeled next. The goal is not merely to train a model, but to train a model \emph{efficiently}, achieving strong generalization while requesting as few annotations as possible \cite{1}. In other words, the learner tries to spend the annotation budget only on those samples that are expected to be most informative for improving the model.
...
\section{Theoretical Foundations of Active Learning}

While active learning is motivated by practical constraints on labeling, it is also supported by strong theoretical results showing that carefully selected samples can significantly reduce the number of labels required to achieve a desired level of accuracy and understand theoretical guarantees on label efficiency and sample selection in active learning \cite{9,10}.

\subsection{Label Complexity vs.\ Sample Complexity}

In traditional supervised learning, the number of labeled samples required to achieve error $\epsilon$ is described by the \textbf{sample complexity}, which often scales as $O(1/\epsilon)$. In contrast, active learning focuses on \textbf{label complexity}, which measures how many labeled samples are needed when the learner can choose which samples to label.

A key theoretical insight is that, under favorable conditions, active learning can achieve the same error rate using exponentially fewer labeled samples. In particular, some active learning algorithms achieve label complexity on the order of $O(\log(1/\epsilon))$, representing a significant improvement over passive learning \cite{10}.

\subsection{Disagreement-Based Learning}

A central theoretical framework for active learning is \textbf{disagreement-based learning}. In this setting, the learner maintains a set of plausible hypotheses (often called the version space) and selects samples for which these hypotheses disagree.
...
This idea motivates algorithms such as \textbf{Query by Committee}, where multiple models are used to identify disagreement regions \cite{11}. Under certain assumptions, these methods can achieve exponential reductions in error with respect to the number of queried labels.

\subsection{Information Gain and Limitations}

Another theoretical perspective connects active learning to \textbf{experimental design}, where the goal is to select samples that maximize expected information gain.

However, maximizing information gain alone does not always guarantee improved predictive performance. Classical results show that if the sampling strategy ignores the underlying data distribution, it may fail to reduce prediction error effectively \cite{11}.
...
\begin{itemize}
    \item Label complexity can be significantly smaller than in passive learning.
    \item Disagreement identifies informative samples.
    \item Effective query strategies must consider both uncertainty and data distribution.
\end{itemize}
...
Several uncertainty-based strategies have been proposed to identify difficult samples. Among the most commonly used approaches are entropy sampling, least confidence sampling, and margin sampling \cite{2,9}. Although these methods differ in how they measure uncertainty, they all share the same objective: selecting samples that are expected to provide the greatest improvement to the model if labeled.
...
This formulation prioritizes samples whose predicted class distributions have the highest entropy. Intuitively, these correspond to samples where the model is most confused and therefore most likely to benefit from additional supervision \cite{2}.
...
\subsection{Relation to Uncertainty, Anomaly Detection, and OOD Detection}
...
In active learning:
\begin{itemize}
    \item Uncertainty is used as a \textbf{query strategy} to improve model performance by selecting samples near the decision boundary.
\end{itemize}

In contrast:
\begin{itemize}
    \item \textbf{Anomaly detection} aims to identify rare or abnormal samples relative to a notion of normality.
    \item \textbf{OOD detection} focuses on identifying samples that do not belong to the training distribution.
\end{itemize}

A key distinction is that uncertain samples in active learning are often \textit{informative but valid} examples (e.g., near class boundaries), whereas anomalies or OOD samples may be irrelevant or even harmful to label and include in the training process. In particular, samples that are out-of-distribution may also exhibit high uncertainty, but querying such samples may not improve the model since they do not reflect the target data distribution.
...
Several diversity-based acquisition strategies have been proposed. Two commonly used approaches are Core Set sampling and discriminative active learning \cite{3}.
...
This greedy procedure provides an efficient approximation to the optimal coverage problem and is widely used in practice because of its simplicity and effectiveness \cite{3}.
...
To quantify performance regression, we define the Negative Flip Rate (NFR) \cite{5}. This metric measures the proportion of samples that were previously classified correctly but become misclassified after the model update.
...
\bibentry{8}
Ash, J. et al.
Deep Batch Active Learning by Diverse Gradient Embeddings.
ICLR, 2020.
