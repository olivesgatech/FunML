...
\lecture{21}{Sequence Modeling III: Transformers}{Ghassan AlRegib and Mohit Prabhushankar}{}

\noindent\textbf{Keywords:} Transformers, self-attention, multi-head attention, positional encoding, cross-attention
...
After establishing the core Transformer components, we examine how these ideas extend beyond language modeling. In particular, we introduce \textbf{Vision Transformers (ViT)}, which adapt the Transformer encoder to image data by representing images as sequences of patches. We study how images are partitioned into fixed-size patches, how these patches are converted into embeddings through linear projection, and how positional embeddings allow the model to preserve spatial structure.

We also discuss how self-attention enables Vision Transformers to capture global spatial relationships between image regions, contrasting this approach with the local receptive fields used in convolutional neural networks (CNNs). This comparison highlights how Transformers provide a unified architecture capable of handling both sequential and visual data.

Finally, we discuss how Transformers generate outputs through the decoding process and how they are trained using standard loss functions.

By the end of this lecture, students should be able to:

\begin{itemize}
\item Explain the motivation behind the Transformer architecture and its advantages over recurrent models
\item Describe how self-attention computes contextual representations
\item Understand the role of Query, Key, and Value projections
\item Explain the purpose of positional encodings, residual connections, and layer normalization
\item Describe how encoder and decoder components interact in sequence modeling tasks
\item Explain how Vision Transformers adapt the Transformer architecture for image processing
\item Understand how patch embeddings allow images to be treated as token sequences
\item Compare Vision Transformers with convolutional neural networks at a high level
\end{itemize}

Overall, students should gain both a conceptual and practical understanding of how Transformers operate and why they have become a dominant architecture across multiple domains of modern machine learning.
...
Having introduced the Transformer architecture for sequence data, we now examine how the same principles can be applied to images. The \textbf{Vision Transformer (ViT)} adapts the Transformer architecture to image data by treating an image as a sequence of patches, analogous to treating a sentence as a sequence of tokens in NLP.

Instead of using convolutional layers, ViT relies entirely on the self-attention mechanism introduced earlier. Each image patch is embedded into a vector representation, combined with positional information, and processed by a Transformer encoder.

Traditional computer vision models such as Convolutional Neural Networks (CNNs) rely on local receptive fields and hierarchical feature extraction. In contrast, Vision Transformers allow every image region to interact with every other region through attention, enabling global reasoning from the first layer. This removes the strong locality bias present in CNNs but allows the model to capture long-range spatial relationships more directly.

\subsection{Patch Embedding (Patchify Step)}
...
where $H$ denotes image height, $W$ denotes image width, and $C$ denotes the number of channels (for RGB images, $C=3$), the image is divided into non-overlapping square patches of size:
...
\begin{figure}[H]
    ...
    \caption{Vision Transformer architecture. The image is divided into patches, embedded into tokens, combined with positional encodings, and processed using a Transformer encoder.}
...
Each patch is then flattened into a vector of dimension:

\[
P^2 C,
\]

which converts the image from a 2D spatial structure into a sequence of vectors. These vectors are projected into a $D$-dimensional embedding space using a learnable linear transformation:
...
This process transforms the image into a sequence of tokens similar to word embeddings used in NLP. Because flattening removes spatial ordering, positional embeddings are added to each patch embedding. These learnable positional vectors encode where each patch originated in the image, allowing the Transformer to retain spatial structure.

A special learnable classification token, denoted \texttt{[CLS]}, is then prepended to the sequence. As the sequence passes through the Transformer encoder, this token aggregates information from all patches and is ultimately used for image classification.

Thus the final token sequence becomes:

\[
[z_{cls}, z_1, z_2, \dots , z_N].
\]

\subsection{Normalization in Vision Transformers}

As discussed earlier in the Transformer architecture, \textbf{Layer Normalization} plays an important role in stabilizing training. Unlike Batch Normalization, which depends on batch statistics, Layer Normalization operates independently on each token by normalizing across feature dimensions. This makes it well suited for Transformer architectures where sequence lengths and batch sizes may vary.
...
y =
\frac{x-\mathbb{E}[x]}
{\sqrt{\text{Var}[x]+\epsilon}}
\cdot \gamma + \beta.
\]

This normalization standardizes each token representation while maintaining learnable scaling and shifting parameters. Because it does not rely on running statistics, Layer Normalization behaves consistently during both training and inference.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{img/lecture21/e068e06a.png}
    \caption{Comparison of Batch Normalization and Layer Normalization. Batch normalization computes statistics across the batch dimension, while layer normalization computes statistics across the feature dimension of each token.}
    \label{fig:layer_norm}
\end{figure}

In Vision Transformers, Layer Normalization is typically applied before attention and feed-forward blocks to maintain stable gradients and improve optimization behavior.

\subsection{Vision Transformer Processing Pipeline}

The complete Vision Transformer processing pipeline can be summarized as follows:

\begin{enumerate}

\item The image is divided into fixed-size patches.
\item Each patch is flattened into a vector.
\item A linear projection converts patches into embeddings.
\item Positional embeddings are added.
\item A CLS token is appended.
\item The sequence is processed by a Transformer encoder.
\item The CLS token representation is used for prediction.

\end{enumerate}

Unlike CNNs, which gradually expand receptive fields through deeper layers, Vision Transformers allow every patch to attend to every other patch immediately through attention. This allows global spatial reasoning from the beginning of the network.

\subsection{Advantages and Limitations}

Vision Transformers provide several important advantages compared to traditional CNN architectures. Because attention operates globally, the model can directly capture long-range spatial relationships between distant image regions. Additionally, ViTs tend to scale effectively with larger datasets and model sizes, often achieving strong performance in large-scale settings. Their architecture is also conceptually simpler because it removes the need to design convolutional filter hierarchies.

However, these benefits come with tradeoffs. Vision Transformers typically require large amounts of training data to outperform CNNs because they lack the strong spatial inductive biases that convolution provides. Additionally, the computational cost of attention grows quadratically with the number of patches, which can make training expensive for high-resolution images. In smaller data regimes, CNNs may still perform better due to their built-in assumptions about spatial locality.

\subsection{Summary}

Vision Transformers extend the Transformer architecture to computer vision by treating images as sequences of patches. Through self-attention, the model can reason globally about spatial relationships. The CLS token enables image-level prediction, while positional embeddings preserve spatial structure. Overall, Vision Transformers trade strong architectural bias for flexibility and scalability, making them particularly effective in large-scale vision tasks.
...
\item \textbf{Question:}
How does a Vision Transformer (ViT) process an image differently from a CNN?

\textbf{Solution:}
ViT divides the image into patches, flattens them, projects them into embeddings, adds positional encodings, and processes the sequence using self-attention. Unlike CNNs, which use local convolutional filters, ViT models global relationships directly via attention.

\item \textbf{Question:}
If an image has size $224 \times 224$ and patch size $P=16$, how many patches are produced?

\textbf{Solution:}
\[
N = \frac{224 \times 224}{16^2}
= \frac{50176}{256}
= 196.
\]
...
\begin{itemize}[nosep]
  ...
  % \item Dosovitskiy, A., Beyer, L., Kolesnikov, A., Weissenborn, D., Zhai, X., Unterthiner, T., Dehghani, M., Minderer, M., Heigold, G., Gelly, S., Uszkoreit, J., \& Houlsby, N. (2021). An image is worth 16x16 words: Transformers for image recognition at scale. \emph{International Conference on Learning Representations (ICLR)}.
