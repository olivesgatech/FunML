...
\begin{figure}[H]
    ...
    \caption{Illustration of label smoothing: a one-hot target distribution is replaced by a softened target distribution that assigns most of the probability mass to the true class while distributing a small amount across the remaining classes.}
...
\begin{figure}[H]
    ...
    \caption{Conceptual illustration of calibration using a reliability diagram. A well-calibrated model has accuracy that closely matches confidence across prediction bins. Temperature scaling adjusts confidence values so that the reliability curve moves closer to the diagonal.}
