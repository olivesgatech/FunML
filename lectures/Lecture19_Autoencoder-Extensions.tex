...
\begin{figure}[H]
    ...
    \caption{Structure of a variational autoencoder. The encoder maps an input \(X\) to the parameters \(\mu\) and \(\sigma\) of the approximate posterior \(q_\theta(Z|X)\). A latent code \(Z\) is sampled from this distribution and passed through the decoder \(p_\phi(X|Z)\) to reconstruct the input.}
\end{figure}
