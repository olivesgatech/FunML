...
%{scriber's name}%%%%%%%%
\lecture{7}{Regression I: Linear Regression}{Ghassan AlRegib and Mohit Prabhushankar}{}
%%%%%CHANGE HERE%%%%%%%
%%%%%\section{title of the section} similarly with the rest \section{} or \

\noindent\textbf{Keywords:} Linear regression, regression, least-squares, SVD, regularization
...
\caption{Figure 7.1: Better Life Index example: GDP per capita vs.\ life satisfaction with a linear fit. Training points are used to fit the line; test points illustrate generalization.}
...
\end{subfigure}
\caption{Figure 7.2: Residual diagnostics for the Better Life Index linear regression model on the training set. Ideally, residuals are roughly centered around zero with no systematic trend versus $\hat{y}$.}
...
\caption{Figure 7.3: Contours of the least squares loss $L(\theta_0,\theta_1)$ for the Better Life Index example (training set). The marked point indicates the minimizer obtained by the normal equation.}
...
\caption{Figure 7.4: 2D linear subspace (green) spanned by \( \begin{pmatrix}1 & 1 & 1\end{pmatrix} \text{ and } \begin{pmatrix}1 & -1 & 1\end{pmatrix} \).}
\end{figure}
...
\caption{Figure 7.5: Projection of \(\mathbf{y}\) onto \(\hat{\mathbf{y}}\), where the residual \(\mathbf{y} - \hat{\mathbf{y}}\) is orthogonal to the subspace.}
\end{figure}
...
\end{subfigure}
\caption{Figure 7.7: Regularization coefficient paths (standardized features). Ridge shrinks all coefficients continuously, while Lasso performs shrinkage plus feature selection by setting some coefficients exactly to zero.}
