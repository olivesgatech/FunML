...
\lecture{25}{Data and Label Efficient Learning I: Self-supervised Learning}{Ghassan AlRegib and Mohit Prabhushankar}{}
...
\noindent\textbf{Keywords:} Self-supervised learning, contrastive learning, representation learning, latent representations, data augmentation
...
Foundation models represent a major shift in modern machine learning \cite{4}. Instead of training task-specific models from scratch, large models are first trained on massive and diverse datasets using \textbf{self-supervised learning} (the \textbf{pre-training stage}), and are later adapted to many downstream tasks.

In this paradigm, pre-training leverages unlabeled data to learn general-purpose representations. The resulting pre-trained model is referred to as a \textbf{foundation model}, which can then be adapted to downstream tasks.

This training pipeline can be summarized in three stages:
\begin{itemize}
\item \textbf{Pre-training (Self-Supervised Learning):} Learn general representations from large-scale unlabeled data
\item \textbf{Fine-tuning:} Adapt the model to specific downstream tasks using labeled data
\item \textbf{Alignment (optional):} Further refine model outputs using human feedback or task-specific constraints
\end{itemize}
...
The impact of foundation models spans many domains, including \textbf{healthcare}, \textbf{embodied interactive perception}, \textbf{visual knowledge distillation}, and \textbf{temporal and commonsense reasoning}. This paradigm reflects the central theme of this lecture: large-scale self-supervised learning enables the development of flexible, general-purpose representations that transfer across tasks.
...
\textbf{Weakly-supervised learning} uses labels that are noisy, imprecise, or less informative than fully supervised labels. These labels are often easier and cheaper to obtain, but introduce uncertainty in the training process. The goal is to learn robust representations despite imperfect supervision.
...
\section{Variations of Self-Supervised Learning Methods}

Different self-supervised learning methods can be understood as modifications to the underlying learning objective. In particular, many contrastive methods can be interpreted as modifying either the \textbf{numerator} or \textbf{denominator} of the InfoNCE loss, while decorrelation-based methods modify the objective to operate across feature dimensions.

\subsection{Variations of Contrastive Methods}
...
\begin{itemize}
\item \textbf{Numerator:} similarity between a positive pair
\item \textbf{Denominator:} similarity between the anchor and all other samples (including negatives)
\end{itemize}

Different methods modify these two components in distinct ways.

\subsubsection*{NNCLR: Modifying the Numerator}

\textbf{NNCLR} (Nearest-Neighbor Contrastive Learning) changes how positive pairs are constructed.

Instead of using only augmented views of the same sample, NNCLR selects a \textbf{nearest neighbor} in the representation space as the positive example. The numerator becomes:
...
\begin{itemize}
\item Expands the notion of similarity beyond strict augmentations
\item Encourages clustering of semantically similar samples
\item Makes the objective more robust to augmentation design
\end{itemize}

Thus, NNCLR can be viewed as modifying the \textbf{numerator of InfoNCE} to incorporate neighborhood structure in the representation space.

\subsubsection*{MoCo v2: Modifying the Denominator}

\textbf{MoCo v2} (Momentum Contrast) modifies how negative samples are constructed.

Instead of relying only on the current mini-batch, MoCo maintains a \textbf{memory queue} of past embeddings. The denominator becomes:
...
\textbf{Interpretation:}
\begin{itemize}
\item Provides a much larger and more diverse set of negatives
\item Improves stability compared to small-batch contrastive learning
\item Approximates a richer estimate of the data distribution
\end{itemize}

Thus, MoCo can be viewed as modifying the \textbf{denominator of InfoNCE} to improve negative sampling.
...
\subsection{Variations of Decorrelation-Based Methods}

Decorrelation-based methods depart from contrastive learning entirely by removing the need for negative samples. Instead of comparing across samples, they impose constraints across \textbf{feature dimensions}.

\subsubsection*{Dimension-Contrastive Perspective}

In contrastive learning, the objective operates across samples:
\[
\text{pull positive samples together, push negative samples apart}
\]

In decorrelation-based methods, the objective operates across feature dimensions:
\[
\text{encourage independence and diversity across features}
\]

This can be viewed as a form of \textbf{dimension-contrastive learning}.

\subsubsection*{Barlow Twins}

\textbf{Barlow Twins} enforces that the cross-correlation matrix between two views is close to the identity matrix.

\textbf{Interpretation:}
\begin{itemize}
\item Diagonal terms enforce invariance (similar representations)
\item Off-diagonal terms enforce decorrelation (independent features)
\end{itemize}

\subsubsection*{BYOL and DINO}

\textbf{BYOL} and \textbf{DINO} avoid collapse without negative samples by using:
\begin{itemize}
\item Teacher–student architectures
\item Momentum updates
\item Implicit regularization through prediction asymmetry
\end{itemize}

\textbf{Interpretation:}
\begin{itemize}
\item These methods rely on architectural asymmetry rather than explicit negative sampling
\item They still implicitly encourage high-dimensional representations
\end{itemize}

\subsubsection*{Connection to Dimensionality}

All decorrelation-based methods can be interpreted as directly enforcing:

\begin{itemize}
\item \textbf{High variance across dimensions}
\item \textbf{Low redundancy between dimensions}
\end{itemize}

This connects directly to the earlier discussion on \textbf{dimensional collapse}, where the goal is to prevent representations from collapsing into a low-dimensional space.
...
\section{Dimensionality in Representation Space}
...
\textbf{Dimensional collapse} occurs when representations map to a low-dimensional subspace or a constant vector, resulting in loss of information and poor downstream performance. This phenomenon is illustrated geometrically in Fig.~\ref{fig:ssl_spherical}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{img/lecture26/ssl_spherical_representation.png}
    \caption{Illustration of representation learning in self-supervised models. 
    Left: collapsed or poorly distributed representations with limited diversity. 
    Right: well-distributed representations across the sphere after training, indicating high-dimensional and information-rich embeddings encouraged by modern self-supervised objectives.}
    \label{fig:ssl_spherical}
\end{figure}
...
\begin{itemize}
\item The \textbf{variance term} ensures each feature dimension maintains sufficient spread
\item The \textbf{covariance term} reduces redundancy between feature dimensions
\end{itemize}

These mechanisms encourage the model to learn \textbf{high-dimensional, information-rich representations}, which are critical for generalization.
...
\section{Domain-Specific Self-Supervised Learning}
...
\subsection{Domain-Specific Pair Construction}
...
\item \textbf{Question:}
What is \textbf{dimensional collapse} in self-supervised learning, and how do modern methods prevent it?

\textbf{Solution:}
Dimensional collapse occurs when learned representations collapse to a low-dimensional space or a constant vector, resulting in loss of information and poor performance on downstream tasks.

Contrastive methods prevent collapse implicitly by pushing apart representations of different samples using negative pairs.

Decorrelation-based methods such as VICReg prevent collapse explicitly by enforcing:
\begin{itemize}
\item Variance: ensuring each feature dimension maintains sufficient spread
\item Covariance: reducing redundancy between features
\end{itemize}

These constraints encourage high-dimensional, information-rich representations.
