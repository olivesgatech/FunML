...
\lecture{22}{Trustworthy ML I: Visual Explainability}{Ghassan AlRegib and Mohit Prabhushankar}{}

\noindent\textbf{Keywords:} Explainability, saliency maps, feature attribution, neural networks, convolutional neural networks
...
which input regions influence a model’s prediction. We then introduce Grad-CAM, a method that produces class-discriminative and spatially meaningful explanations by leveraging gradients in the final convolutional layer. Finally, we introduce model-agnostic explanation methods including Local Interpretable Model-agnostic Explanations (LIME) and SHapley Additive exPlanations (SHAP), which provide feature-level explanations applicable to a wide range of machine learning models. Together,
...
We begin by examining a classical convolutional neural network (CNN) architecture, AlexNet \cite{1}.
...
\paragraph{}
AlexNet contains eight learnable layers \cite{1}:
...
\textbf{Key takeaway:} AlexNet contains approximately \textbf{60 million parameters} \cite{1},
...
\begin{itemize}
    \item Uses deeper architectures with identity shortcuts and convolutional blocks to enable training of very deep networks \cite{3}.
...
As we move deeper into a CNN, the learned representations become increasingly abstract and harder to interpret directly \cite{2}. While first-layer filters often resemble simple edge or color detectors, intermediate layers combine these primitive features into more complex and semantically meaningful patterns.
...
Deeper convolutional layers begin to respond to \textbf{object parts} and higher-level semantic concepts rather than simple edges \cite{2}. For example, the fifth convolutional layer (Conv5) in networks such as AlexNet is particularly sensitive to specific object components, such as wheels. This selectivity is visualized through activation maps, which highlight spatial regions where a filter responds most strongly. These activation maps reveal which parts of the image contribute most to the network’s internal representation and ultimately influence the final prediction.
...
Beyond visualizing intermediate activations, we can analyze which specific regions of input images cause the strongest responses in individual neurons. \textbf{Maximally activating patches} refer to image regions that trigger the highest activation values for a particular filter within a network \cite{2}. By identifying these regions, we gain insight into what visual patterns a neuron is most sensitive to and what semantic features it has learned to detect.
...
To make these high-dimensional representations interpretable, dimensionality reduction techniques are applied. Common approaches include Principal Component Analysis (PCA), Isomap, Uniform Manifold Approximation and Projection (UMAP), and t-Distributed Stochastic Neighbor Embedding (t-SNE) \cite{4}. These methods project the feature vectors from thousands of dimensions down to 2D or 3D, enabling visualization of the learned feature space where each point corresponds to one image.
...
t-SNE is particularly effective for visualization because it preserves \textbf{local neighborhood relationships} \cite{4}. As a result, images that share similar visual features form clear clusters in the projected space. These clusters often correspond to semantic categories, making it easier to interpret the structure of the dataset and the behavior of the model.
...
A major drawback of occlusion-based methods is their \textbf{computational cost} \cite{2}. To evaluate the importance of different regions, many occluded versions of the image must be processed by the network. For high-resolution images, this requires a large number of forward passes, making the method expensive in practice.
...
\begin{figure}[H]
    ...
    \caption{Occlusion-based saliency heatmaps}
...
Gradient-based saliency provides a more efficient alternative to occlusion methods for estimating pixel importance \cite{5}. Instead of masking parts of the image repeatedly, we compute how sensitive the model’s prediction is to small changes in each input pixel. The resulting gradient-based saliency map highlights which pixels most strongly influence the output.
...
Although the full network is highly non-linear, we can understand its local behavior using a first-order Taylor approximation around a specific input \cite{5}:
...
A key advantage of gradient-based saliency is computational efficiency \cite{5}. Gradients can be computed using backpropagation in a single forward–backward pass, making the method practical even for high-dimensional images.
...
\begin{figure}[htbp]
    ...
    \caption{Forward pass through the CNN}
...
Vanilla backpropagation is the most direct approach for generating saliency maps \cite{5}. Given an input image and a target class score, we compute the gradient of the score with respect to the input pixels. The magnitude of this gradient indicates how strongly small changes in each pixel would affect the prediction.
...
Deconvnet backpropagation is a modified gradient-based visualization method designed to produce clearer and more interpretable saliency maps than vanilla backpropagation \cite{2}. The key idea is to change how gradients are propagated through ReLU non-linearities during the backward pass.
...
\begin{figure}[H]
    ...
    \caption{ReLU activation in the forward and backward pass.}
...
Although guided backpropagation produces visually appealing and sharp saliency maps, it has several important limitations \cite{6,8}. In particular, the method often lacks \textbf{class discriminativity} and may highlight image structures that are not truly responsible for the model’s prediction.
...
These limitations motivate the development of more advanced techniques (such as Grad-CAM) that explicitly incorporate class information when generating explanations.
 ...
 \begin{figure}[H]
     ...
     \caption{Example of noisy gradient-based saliency maps.}
...
they often suffer from noise and poor class discriminativity. Grad-CAM was introduced to produce \emph{class-specific} visual explanations that are both interpretable and spatially meaningful \cite{8}.
...
Grad-CAM computes a class-specific heatmap by combining feature maps using weights derived from gradients of the target class score \cite{8}.
...
\begin{figure}[H]
    ...
    \caption{Grad-CAM produces class-discriminative localization maps that
...
\begin{figure}[H]
    ...
    \caption{Grad-CAM computation pipeline: forward pass, gradient
    backpropagation, global-average pooling of gradients, and weighted
    combination of feature maps.}
    \label{fig:gradcam_process_l22}
\end{figure}

Together, these steps allow Grad-CAM to connect internal feature representations to human-interpretable image regions \cite{8}. Compared to Guided Backpropagation, which
...
This property makes Grad-CAM an effective tool for debugging models, detecting failure modes, and building trust in neural network predictions \cite{8}.



\section{Model-Agnostic Explanation Methods}

While visualization methods such as saliency maps and Grad-CAM are primarily designed for convolutional neural networks, many explainability techniques are designed to work with any machine learning model. These are known as \textbf{model-agnostic explanation methods}. Two widely used approaches in this category are LIME and SHAP.

\subsection{Local Interpretable Model-agnostic Explanations (LIME)}

\textbf{LIME} (Local Interpretable Model-agnostic Explanations) is an explainability technique that approximates a complex model locally using a simpler interpretable model such as a linear model \cite{9}. The key idea is that while a global model may be highly complex, its behavior around a single prediction can often be approximated by a simple surrogate model.

\subsubsection{Key Idea}

LIME explains a prediction by perturbing the input data and observing how the prediction changes. A simple interpretable model is then trained on these perturbed samples to approximate the behavior of the original model locally. This allows the method to identify which features were most influential for that specific prediction.

The process typically involves selecting an instance to explain, generating perturbed samples near that instance, obtaining predictions from the original model, and weighting samples based on their similarity to the original input. A simple interpretable model such as linear regression is then fit to this locally weighted dataset to estimate feature importance. This process is illustrated in Figure~\ref{fig:lime}.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{img/lecture22/lime_pipeline.png}
\caption{Illustration of the LIME process for generating local explanations by sampling perturbations and fitting an interpretable surrogate model.}
\label{fig:lime}
\end{figure}

\subsubsection{Method}

Given an input instance $x$ to be explained, LIME constructs a local approximation of the model by generating a dataset of perturbed samples around $x$. These samples are obtained by randomly modifying features of the input, such as removing words in text data or masking regions in images. 

Each perturbed sample is then passed through the original black-box model to obtain predictions. To ensure that the explanation focuses on the local neighborhood of $x$, LIME assigns higher weights to samples that are closer to the original input and lower weights to those that are farther away.

Using this weighted dataset, LIME fits a simple interpretable model, typically a sparse linear model, to approximate the behavior of the black-box model in the vicinity of $x$. The coefficients of this surrogate model are then used as feature importance scores, providing an explanation for the prediction at $x$.

This procedure is illustrated in Figure~\ref{fig:lime}.

\subsubsection{Advantages and Limitations}

One advantage of LIME is that it can be applied to any machine learning model without requiring access to internal model parameters. It also produces intuitive feature importance scores that can help practitioners understand and debug model behavior. 

However, LIME explanations are only locally valid and may not generalize to the model's global behavior. Additionally, results may vary depending on the sampling procedure, and the need to generate many perturbations can increase computational cost.

\subsection{SHapley Additive exPlanations (SHAP)}

\textbf{SHAP} (SHapley Additive exPlanations) is a unified framework for explainability based on Shapley values from cooperative game theory \cite{10}. SHAP assigns an importance value to each feature by measuring its contribution to the prediction relative to a baseline expectation.

\subsubsection{Key Idea}

SHAP treats each feature as a player in a cooperative game and computes how much each feature contributes to the final prediction by averaging its marginal contribution across all possible feature subsets. This produces theoretically grounded feature importance values with desirable consistency properties.

The SHAP explanation model can be written as:

\[
f(x) = \phi_0 + \sum_{i=1}^{M} \phi_i
\]

where $\phi_i$ represents the contribution of feature $i$, $\phi_0$ represents the base prediction, and $M$ is the number of features.

This process is illustrated in Figure~\ref{fig:shap}.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{img/lecture22/shap.png}
\caption{Example SHAP explanation showing how feature contributions shift the prediction from a baseline value.}
\label{fig:shap}
\end{figure}

\subsubsection{Method}

Given an input $x$, SHAP computes the contribution of each feature by evaluating how the prediction changes when that feature is added to different subsets of features. For each feature, the method considers all possible subsets of the remaining features and measures the marginal contribution of including that feature.

These contributions are then averaged across all subsets to obtain the Shapley value for each feature. This value represents the expected contribution of the feature to the prediction, ensuring a fair allocation of importance across all features.

Because evaluating all possible feature subsets is computationally expensive, practical implementations of SHAP rely on approximation methods or model-specific optimizations to efficiently estimate these values.

This additive decomposition is illustrated in Figure~\ref{fig:shap}.

\subsubsection{Advantages and Limitations}

SHAP has a strong theoretical foundation and provides consistent feature attribution, making it attractive for both local and global explanation tasks. It also provides a unified framework connecting several existing explanation methods.

However, exact computation of Shapley values is computationally expensive, particularly for models with many features. As a result, practical implementations typically rely on approximations.

\subsection{LIME vs SHAP}

Both LIME and SHAP provide feature-level explanations for black-box models but differ in their methodology. LIME approximates model behavior locally using surrogate models, while SHAP computes feature contributions using a theoretically grounded game-theoretic framework. In practice, LIME is often faster and simpler to apply, while SHAP provides more consistent and theoretically justified explanations.

Together, these approaches provide powerful tools for interpreting complex machine learning models and improving model transparency and trust.
...
\item \textbf{Question:}
Why is LIME considered a local explanation method rather than a global explanation method?

\textbf{Solution:}
LIME approximates the behavior of a complex model only in the neighborhood of a specific input by fitting a simple surrogate model to perturbed samples. Because this approximation is valid only near that instance, it does not necessarily reflect the model’s behavior across the entire dataset. Therefore, LIME provides local explanations rather than global interpretability.

\item \textbf{Question:}
Why does LIME generate perturbed samples when constructing explanations?

\textbf{Solution:}
LIME generates perturbed samples to observe how small changes in the input affect the model’s prediction. By analyzing how predictions vary with these perturbations, LIME can estimate which features are most influential for that specific prediction. This allows the method to approximate the model’s local decision boundary.

\item \textbf{Question:}
What is the main theoretical advantage of SHAP over LIME?

\textbf{Solution:}
SHAP is based on Shapley values from cooperative game theory, which provide a theoretically grounded way to fairly attribute feature importance. These values satisfy desirable properties such as consistency and additivity. In contrast, LIME relies on locally fitted surrogate models and does not guarantee these theoretical properties.

\item \textbf{Question:}
Why is computing exact SHAP values often computationally expensive?

\textbf{Solution:}
Computing exact Shapley values requires evaluating the contribution of each feature across all possible subsets of features. Since the number of subsets grows exponentially with the number of features, exact computation becomes intractable for high-dimensional data. Practical implementations therefore rely on approximations.

\item \textbf{Question:}
Compare LIME and SHAP in terms of explanation stability.

\textbf{Solution:}
LIME explanations may vary depending on the randomly generated perturbations used to construct the local surrogate model, which can reduce stability. SHAP explanations tend to be more stable because they are derived from a principled attribution framework based on Shapley values, although approximations may still introduce some variability.

\item \textbf{Question:}
When might LIME be preferred over SHAP in practice?

\textbf{Solution:}
LIME may be preferred when computational efficiency is important, since it typically requires fewer computations than SHAP. It may also be preferred when quick local explanations are sufficient and theoretical guarantees are less critical.
