...
\lecture{16}{Convolutional Neural Networks V: Best Practices}{Ghassan AlRegib and Mohit Prabhushankar}{}

\noindent\textbf{Keywords:} Best practices, classification, calibration, data augmentation, regularization, cross-validation, training dynamics
...
\begin{figure}[H]
    ...
    \caption{Figure 16.1: Classification Pipeline}
\label{0}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.2: Dense prediction architecture using mainly convolution}
    \label{1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.3: Effect of $\gamma$ on focal loss}
    \label{2}
\end{figure}


\section{Confidence Regularization and Calibration}

In classification tasks, strong predictive accuracy alone is often not sufficient. A model may achieve high accuracy while still producing probabilities that are too confident relative to its true correctness likelihood. This issue is important in downstream decision-making systems because predicted probabilities are often used for thresholding, uncertainty estimation, or integration into larger pipelines. Two common techniques for addressing confidence-related issues are \textbf{label smoothing} and \textbf{temperature scaling}. Although they are related, they are applied at different stages of the learning pipeline and serve slightly different purposes.

Label smoothing is used \textbf{during training} as a regularization method that prevents the network from becoming overly certain about the correct class. Temperature scaling is typically used \textbf{after training} as a post-processing calibration step that adjusts predicted probabilities without changing the predicted class labels. Together, these methods provide practical tools for improving the reliability of classification models.

\subsection{Label Smoothing}

\textbf{Label smoothing} is a regularization technique for classification models trained with cross-entropy loss. Instead of using a hard one-hot target vector, label smoothing replaces the target distribution with a softened version that allocates most of the probability mass to the true class while assigning a small amount of probability mass to the remaining classes. This discourages the model from driving the predicted probability of the correct class arbitrarily close to 1 and helps reduce overconfidence.
...
\subsubsection*{Why Label Smoothing Helps}
...
\begin{itemize}
    \item It reduces overconfidence in the output probabilities.
    \item It improves generalization by acting as a regularizer.
    \item It can improve calibration, meaning the predicted probabilities better reflect actual correctness likelihoods.
    \item It may help when labels are noisy or when classes have some semantic similarity.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\linewidth]{img/lecture17/label_smoothing_targets.png}
    \caption{Figure 16.4: Illustration of label smoothing: a one-hot target distribution is replaced by a softened target distribution that assigns most of the probability mass to the true class while distributing a small amount across the remaining classes.}
    \label{fig:label_smoothing}
\end{figure}

\subsection{Temperature Scaling}

\textbf{Temperature scaling} is a post-processing calibration method applied after a classifier has already been trained. Unlike label smoothing, it does not change the learned class boundaries or retrain the model weights. Instead, it rescales the logits before the softmax function using a single positive scalar parameter \(T\), called the \textbf{temperature}.
...
\subsubsection*{Why Temperature Scaling Helps}

Modern deep neural networks often produce probabilities that are too extreme even when the final class prediction is correct. For example, a model may assign confidence \(0.95\) to predictions that are actually correct only \(80\%\) of the time. In such a case, the classifier is said to be \textbf{overconfident}. Temperature scaling addresses this by shrinking the logit magnitudes before the softmax is applied, thereby reducing confidence without changing the ranking of the logits.

An important practical property of temperature scaling is that it \textbf{does not change the predicted class label}. Since dividing all logits by the same positive scalar preserves their ordering, the argmax remains unchanged:
...
\subsubsection*{Reliability Diagrams and Calibration Error}

Calibration is often analyzed using \textbf{reliability diagrams}, which compare predicted confidence against empirical accuracy across bins of predictions. A perfectly calibrated model would have confidence values that match observed accuracy, so its reliability curve would lie near the diagonal. If the curve lies below the diagonal, the model is overconfident; if it lies above the diagonal, the model is underconfident.

A common scalar summary of calibration quality is the \textbf{Expected Calibration Error (ECE)}, defined as
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{img/lecture17/temperature_scaling_reliability.png}
    \caption{Figure 16.5: Conceptual illustration of calibration using a reliability diagram. A well-calibrated model has accuracy that closely matches confidence across prediction bins. Temperature scaling adjusts confidence values so that the reliability curve moves closer to the diagonal.}
    \label{fig:temperature_scaling}
\end{figure}

\subsection{Relationship Between Label Smoothing and Temperature Scaling}

Although both methods aim to reduce overconfidence, they are used in different ways.

\begin{itemize}
    \item \textbf{Label smoothing} is a \textbf{training-time regularization method}. It changes the target distribution and influences how the model learns.
    \item \textbf{Temperature scaling} is a \textbf{post-training calibration method}. It leaves the trained classifier unchanged and only adjusts the output probabilities.
\end{itemize}
...
\begin{itemize}
    \item Use \textbf{label smoothing} when the classifier appears to overfit or become overly confident during training.
    \item Use \textbf{temperature scaling} when the model is already trained and you want better-calibrated probabilities without changing its accuracy.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.6: Image Reconstructions and their Performance Scores}
    \label{3}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.7: Underfitting, Proper Fitting, and Overfitting}
    \label{4}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.8: Examples of Transformations Applied to an Image}
    \label{fig:5}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.9: Activation Histogram with Dead Neurons and Vanishing Gradients}
    \label{fig:6}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.10: Activation Histogram with Exploding Gradients}
    \label{fig:7}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.11: Ideal Activation Histogram}
    \label{fig:8}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Figure 16.12: Activation Histogram with Outliers}
    \label{fig:9}
\end{figure}
